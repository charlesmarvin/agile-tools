export default {
  ...
  methods: {
    createBoard: async function (event) {
      const vm = this
      const payload = {
        name: vm.boardName,
        type: vm.boardType,
      ...
      if (vm.passcodeProtect) {
        payload.passcode = vm.passcode
      }
      console.log('creating board ', payload)
      // FIXME refactor validation of form steps
      if (!(payload.name && payload.type && payload.email)) {
        vm.$root.$emit('alert-error', 'Name, Type, and Email are required.')
        return
      }
      return fetch('/api/v1/boards', {
        ...
        .then(res => {
          ...
          const navigationProps = { name: 'board', params: { id: res.id } }
          if (vm.passcodeProtect) {
            navigationProps.query = { passcode: vm.passcode }
          }
          this.$router.push(navigationProps)
        })
    ...
    next: function () {
      const vm = this
      // FIXME refactor validation of form steps
      if (vm.boardName && vm.boardType &&
        (!vm.passcodeProtect || (vm.passcodeProtect && vm.passcode))) {
        vm.step += 1
      } else {
    ...
    toggleSecure: function () {
      const vm = this
      vm.passcodeProtect = !vm.passcodeProtect
    }
  },
  template: `
    <form id="create-board-form">
      <div v-if="step == 1">
        ...
        <div class="flex-grid-sp">
          <div class="col">
            <button @click.prevent="next" class="btn" type="submit">Next</button>
          </div>
        </div>
      </div>
      <div v-if="step == 2">
        <div class="flex-grid sp-bottom">
          <div class="col">
            <input v-model="email" type="text" name="email" placeholder="Email address" id="board-owner-email">
          </div>
        </div>
